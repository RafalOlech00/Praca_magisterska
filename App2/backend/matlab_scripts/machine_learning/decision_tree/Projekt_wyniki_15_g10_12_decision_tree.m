function result = Projekt_wyniki_15_g10_12_decision_tree()
clc;
clear variables;

% Wczytanie danych 
[X, Y] = ReadData_wyniki_15_g10_12_decision_tree();
...
%[Y_predT] = TreeModel(X_train, X_test, Y_train, Y_test);
[Y_predT] = TreeModel2(X_train, X_test, Y_train, Y_test);
%[Y_predT, bestParams] = TreeModelParams(X_train, X_test, Y_train, Y_test);
...
title('Actual vs Predicted Welding Temperature (Decision Tree) - 10-12');
xlabel('Sample');
ylabel('Temperature');
hold off;
...
while predicted_index <= length(Y_predT) || actual_index <= length(Y_test)
    elapsed_time = toc(start_time);  % Obliczanie upływu czasu
    
    % Rysowanie linii 'Predicted' 
    if predicted_index <= length(Y_predT)
        set(h2, 'XData', 1:predicted_index, 'YData', Y_predT(1:predicted_index));
