function result = Projekt_wyniki_15_g10_12_lstm()
clc;
clear variables;

% Wczytanie danych
[X, Y] = ReadData_wyniki_15_g10_12_lstm();
...
% LSTM
fprintf('LSTM \n');
%[best_params, best_net, best_info, Y_predLSTM] = LSTM(X_train, X_test, Y_train, Y_test);
[best_params, best_net, best_info, Y_predLSTM] = LSTMBestParams(X_train, X_test, Y_train, Y_test);
...
title('Actual vs Predicted Welding Temperature (LSTM) - 10-12');
xlabel('Sample');
ylabel('Temperature');
hold off;
...
while predicted_index <= length(Y_predLSTM) || actual_index <= length(Y_test)
    elapsed_time = toc(start_time);  % Obliczanie upływu czasu
    
    % Rysowanie linii 'Predicted' 
    if predicted_index <= length(Y_predLSTM)
        set(h2, 'XData', 1:predicted_index, 'YData', Y_predLSTM(1:predicted_index));
